...
| **Move the dock** | **Drag the chevron up or down** to move the whole HUD along the right edge — ten notches over the height of the screen. Notched rather than free, because at a hundred possible positions you spend effort placing it and the point of the dock is that you stop thinking about it. **Past halfway it turns over**: the dock hangs from its anchor instead of standing on it, the tabs grow downward, and the usage meter moves below them. That isn't decoration — the stack grows *away* from its anchor, so without the flip a dock dragged high would grow straight off the top of the screen. A short press still stows; only a real drag moves it. |
...
The panel shows a **sparkline of how fast you're spending it** — the rate, not the total, drawn from the transcripts rather than from the plan's percentage. That matters for resolution: the percentage arrives in whole points no oftener than every 45 seconds, so a single turn is invisible until it happens to tip a point over, whereas every call is on disk with its real timestamp and the tailer sees it within five seconds. Five-second buckets with a short smoothing kernel, so one question you just asked shows up as its own bump. It measures spend on **this machine**; anything running elsewhere is in the Elsewhere row instead, which is why the two can disagree. How full the window is is what the bar's own length says; a chart of the same thing answers the wrong question and never comes back down, so an idle hour reads as "still climbing". This one sits on zero when nothing is happening, so "on pace for 46%" comes
